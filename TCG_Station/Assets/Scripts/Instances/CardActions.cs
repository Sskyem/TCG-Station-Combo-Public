...
public class CardActions : MonoBehaviour
{
                                     ...
                                     AttackData attack)
    {
        ...
        if (!CanPayAttackAdditionalCosts(attackerOwner, attack))
        {
        ...
        BattleManager.Instance.BeginCombatResolution();
                                               ...
                                               AttackEffectContext context, int attackDamage)
    {
        ...
        finally
        {
            if (BattleManager.Instance != null)
                BattleManager.Instance.EndCombatResolution();
        }
    }
            ...
            switch (effect.cardEffectType)
            {
                ...
                case EnumCardEffectType.Multiattack:
                    if (currentAttack != null && sourcePokemon != null && targetPlayer != null)
                    {
                        ...
                        {
                            if (sourcePokemon.currentHp <= 0)
                            {
                                Debug.Log("[Multiattack] Stopping extra hits: attacker is no longer in play.");
                                break;
                            }
                            ...
                            // Multiattack repeats the hit. Recoil expressed as DealDamage(Self)
                            // belongs to every trigger as printed on Pinsir/Hydreigon, not only
                            // to the first hit.
                            ApplyRepeatedMultiattackSelfDamage(sourcePokemon, sourcePlayer, effects);

                            if (sourcePokemon.currentHp <= 0)
                            {
                                Debug.Log("[Multiattack] Stopping extra hits: attacker was knocked out by self-damage.");
                                break;
                            }
                        }
                    }
    ...
    public static bool CanPayAttackAdditionalCosts(PlayerController player, AttackData attack)
    {
        if (player == null || attack == null) return false;
        return player.hand.Count >= GetDiscardCost(player, attack.effects);
    }
    ...
    private static void ApplyRepeatedMultiattackSelfDamage(
        Pokemon sourcePokemon,
        PlayerController sourcePlayer,
        IEnumerable<EffectData> effects)
    {
        if (sourcePokemon == null || sourcePlayer == null || effects == null) return;

        foreach (EffectData effect in effects)
        {
            if (effect.cardEffectType == EnumCardEffectType.DealDamage &&
                effect.cardEffectTarget == EnumCardEffectTarget.Self &&
                effect.effectAmount > 0)
            {
                BattleManager.Instance.ApplyEffectDamage(sourcePokemon, sourcePlayer, effect.effectAmount);
                if (sourcePokemon.currentHp <= 0)
                    return;
            }
        }
