...
public class VisualCard : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    [SerializeField, Range(0f, 1f)] float pointerSoundVolume = 1f;
    ...
    private void Awake()
    {
        ...
        pointerAudioSource = GetComponent<AudioSource>();
        if (pointerAudioSource == null)
        {
        ...
        if (pointerSound != null &&
            pointerSound.loadState == AudioDataLoadState.Unloaded)
        {
            pointerSound.LoadAudioData();
        }
    }
    ...
    private void PlayPointerSound()
    {
        ...
        if (pointerSound.loadState == AudioDataLoadState.Unloaded &&
            !pointerSound.LoadAudioData())
        {
            return;
        }

        if (pointerSound.loadState == AudioDataLoadState.Failed)
        {
            Debug.LogWarning($"[VisualCard] Failed to load pointer sound '{pointerSound.name}'.", this);
            return;
        }
