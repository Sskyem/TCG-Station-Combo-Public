...
public class GameRulesConfig : MonoBehaviour
{
    ...
    [Tooltip("Base URL of the remote log receiver.")]
    public string logServerUrl = "";

    [Tooltip("API key sent to the remote log receiver in the X-Api-Key header.")]
    public string logServerApiKey = "";
    ...
    private void LoadFromJson()
    {
        ...
        try
        {
            ...
            logServerUrl = Get(obj, "logServerUrl", logServerUrl);
            logServerApiKey = Get(obj, "logServerApiKey", logServerApiKey);
    ...
    private void ValidateValues()
    {
        ...
        logServerUrl = (logServerUrl ?? "").Trim();
        logServerApiKey = (logServerApiKey ?? "").Trim();
    }
    ...
    public void SaveToJson()
    {
        ...
        JObject obj = new JObject
        {
            ...
            ["logServerApiKey"] = logServerApiKey,
        ...
        if (File.Exists(path))
        {
            var updates = new Dictionary<string, JToken>();
            foreach (JProperty property in obj.Properties())
                updates[property.Name] = property.Value;

            if (JsoncConfigWriter.TryUpdateFile(path, updates, out string error))
            {
                Debug.Log($"[GameRulesConfig] Zapisano do {path}, zachowując komentarze.");
                return;
            }

            Debug.LogError($"[GameRulesConfig] Nie udało się zapisać JSON: {error}");
            return;
        }
        ...
        try
        {
            ...
            Debug.Log($"[GameRulesConfig] Utworzono {path}");
        }
        catch (Exception e)
        {
            Debug.LogError($"[GameRulesConfig] Nie udało się utworzyć JSON: {e.Message}");
        }
    }
    ...
    public bool SaveStartupMenuSettingsToJson()
    {
        ...
        Debug.LogError($"[GameRulesConfig] Nie udało się zapisać ustawień menu: {error}");
        return false;
    }
}
