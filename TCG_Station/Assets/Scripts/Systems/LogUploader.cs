...
/// Requires logUploadEnabled=true and remote receiver settings in GameRulesConfig.
...
public class LogUploader : MonoBehaviour
{
    ...
    public IEnumerator UploadAfterLogging()
    {
        ...
        var config = GameRulesConfig.Instance;
        if (config == null ||
            string.IsNullOrWhiteSpace(config.logServerUrl) ||
            string.IsNullOrWhiteSpace(config.logServerApiKey))
        {
            Debug.LogWarning(
                "[LogUploader] Upload enabled, but GameRulesConfig.json is missing logServerUrl or logServerApiKey.");
            yield break;
        }

        string gameId  = record.game_id;
        string baseUrl = config.logServerUrl.TrimEnd('/');
        string apiKey  = config.logServerApiKey;
