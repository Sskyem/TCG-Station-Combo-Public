...
All runtime settings live in `GameRulesConfig.json`. Use `Custom` for the matching preset when
supplying a non-local ML or Ollama URL.
...
2. **Presets** — `mlServerPreset` / `ollamaEndpointPreset`: unless set to `Custom`, they overwrite `mlServerUrl` / `ollamaBaseUrl` with localhost.
...
"logServerUrl": "https://logs.example.invalid",
"logServerApiKey": "replace-me"
```

The private repository may contain the real receiver values. Public exports replace them with
disabled, empty values in the exported copy of `GameRulesConfig.json`.
