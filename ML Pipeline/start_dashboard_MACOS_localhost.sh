...
ML_ROOT="$SCRIPT_DIR"
COMBO_ROOT="$(cd "$ML_ROOT/.." && pwd)"
GAME_ROOT="$COMBO_ROOT/TCG_Station"
cd "$ML_ROOT"
...
SERVE_ARGS=(--host 127.0.0.1 --port 8000 --ml-root "$ML_ROOT")
if [ -d "$GAME_ROOT" ]; then
  SERVE_ARGS+=(--root "$GAME_ROOT")
fi

open "http://localhost:8000" 2>/dev/null || true
"$VENV_PYTHON" scripts/serve.py "${SERVE_ARGS[@]}"
